...
public class FootballGame2D implements MouseListener,MouseMotionListener{
       
    GraphicsBuilder gb
    def panel;
    FootballGamePlayManager gamePlay;
    SoccerPitch pitch;
    ...
    public FootballGame2D(){
        gb = new GraphicsBuilder();
    }
    
    public void init(){
        expandoClasses();
        gamePlay = FootballGamePlayManager.getDefault();
        pitch = gamePlay.getPitch();
    ...
    void updateGamePlay(){
        ...
        if (debugTeam){
            println pitch.getRedTeam().getFSM().getCurrentStateName();
            pitch.getRedTeam().debugTeam();
            println " -----------------------------------------------";
        }
    }
    ...
    public void mouseClicked( MouseEvent e ){
        if (e.getButton() == e.BUTTON1){
            /*
            pitch.getRedTeam().getMembers().each{player->
